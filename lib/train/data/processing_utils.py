...
def transform_image_to_crop_multi(box_in: torch.Tensor, crop_sz: torch.Tensor, output_sz, search_area_factor, normalize=False) -> torch.Tensor:
    """ Transform the box co-ordinates from the original image co-ordinates to the co-ordinates of the cropped image
    args:
        box_in - the box for which the co-ordinates are to be transformed
        box_extract - the box about which the image crop has been extracted.
    ...
    returns:
        torch.Tensor - transformed co-ordinates of box_in
    """
    def transform_single_box(box, extract, factor, crop_size):
        # 원래 이미지와 크롭 이미지의 중심점 계산
        extract_center = extract[0:2] + 0.5 * extract[2:4]
        box_center = box[0:2] + 0.5 * box[2:4]
        
        # 크롭 이미지에서의 중심점 계산
        out_center = (crop_size - 1) / 2 + (box_center - extract_center) * factor
        
        # 크롭 이미지에서의 너비와 높이 계산
        out_wh = box[2:4] * factor
        
        # 크롭 이미지에서의 상대적인 좌표 계산
        out_box = torch.cat((out_center - 0.5 * out_wh, out_wh))
        return out_box

    # 각 좌표를 변환하여 리스트에 추가
    transformed_boxes = []
    for i in range(0, len(box_in), 4):
        box = box_in[i:i+4]
        w = box[2]
        h = box[3]
        # Resize factor
        cs = math.ceil(math.sqrt(w*h) * search_area_factor)
        os = output_sz
        resize_factor = os/cs
        transformed_box = transform_single_box(box, box, resize_factor, crop_sz)
        transformed_boxes.append(transformed_box)

    # 결과를 하나의 텐서로 변환
    transformed_boxes = torch.stack(transformed_boxes)
    transformed_boxes = transformed_boxes.reshape(1, -1).squeeze()
    
    if normalize:
        return transformed_boxes / crop_sz[0]
    else:
...
def pre_center_crop(pre_box_gt, search_area_factor, output_sz) :
    # frames_crop: tuple of ndarray (128,128,3), att_mask: tuple of ndarray (128,128)
    crop_sz = torch.Tensor([output_sz, output_sz])
    # find the bb location in the crop
    '''Note that here we use normalized coord'''
    pre_box_crop = [transform_image_to_crop_multi(a_gt, crop_sz, output_sz, search_area_factor, normalize=True)
                    for a_gt in pre_box_gt]
    return pre_box_crop
