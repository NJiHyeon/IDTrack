...
class STARKProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        """
        # search_anno : [tensor([ 422.,  293., 1151.,  524.])]
        #print('d1', data['template_images'][0].shape) : [1080, 1920, 3], [720, 1280, 3], ...
        
        ## Apply joint transforms
        # 결국 self.transform[] : tfm.Transform(tfm.ToTensor(~~))
        # self.transform['joint'] : None임
        if self.transform['joint'] is not None:
            ...
            data['search_images'], data['search_anno'], data['search_masks'], data['search_pre_anno']  = self.transform['joint'](
                image=data['search_images'], bbox=data['search_anno'], mask=data['search_masks'], prev_bbox=data['search_pre_anno'], new_roll=False)
        ...
        for s in ['template', 'search']:
            ...
            '''여기서 pre_bbox 처리..'''
            '''prutils.jittered_center_crop 여기 함수에서 pre_bbox 처리 기법ㅂ 만들기'''
            """""""소수로만듦"""""""
            ...
            if s == 'template' :
                ...
                prev_bboxes = prutils.pre_center_crop(data[s + '_pre_anno'], self.search_area_factor[s], self.output_sz[s])
                data[s + '_images'], data[s + '_anno'], data[s + '_att'], data[s + '_masks'], data[s + '_pre_anno'] = self.transform[s](
                image=crops, bbox=boxes, att=att_mask, mask=mask_crops, prev_bbox=prev_bboxes, joint=False)
