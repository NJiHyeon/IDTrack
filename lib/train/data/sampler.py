...
class TrackingSampler(torch.utils.data.Dataset):
    """ Batch를 형성하기 위해 training sequence에서 frame을 sampling하는 작업을 담당하는 클래스
    Sampling은 다음과 같은 방법으로 이루어진다.
    1. 먼저 Data set가 무작위로 선택된다.
    2. 그런 다음 해당 Data set에서 sequence를 선택한다.
    3. 그런 다음 base frame이 sequence로부터 무작위로 sampling된다.
    4. [base_frame_id - max_gap, base_frame_id]에서 'train frame'을 샘플링
       [base_frame_id, base_frame_id + max_gap]에서 'test frame'을 샘플링
    * target이 보이는 frame만 샘플링된다.
    * 보이는 frame이 충분히 발견되지 않으면, 'max_gap'은 충분한 frame이 발견될 때까지 점진적으로 증가한다.
    """
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
        ...
        args:
            ...
            frame_sample_mode - Either 'causal' or 'interval'. 
                                (If 'causal', then the test frames are sampled in a causally, otherwise randomly within the interval.)
        """
    ...
    def _sample_visible_ids(self, visible, num_ids=1, min_id=None, max_id=None, 
        ...
        """ target이 visible한 num_ids frames을 min_id와 max_id 사이에서 샘플링한다.
        args:
    ...
    def getitem(self):
        ...
        while not valid:
            # 1. Select a dataset
            dataset = random.choices(self.datasets, self.p_datasets)[0] #첫번째
            
            is_video_dataset = dataset.is_video_sequence()
            
            # 2. Sample a sequence from the given dataset
            seq_id, visible, seq_info_dict = self.sample_seq_from_dataset(dataset, is_video_dataset)
            
            # 3. Make frame & Data dict
            # 3-1. video set
            if is_video_dataset:
            ...
            # 3-2. image set (just repeat the image to generate synthetic video)
            else:
            ...
            try:
                ...
                data = TensorDict({'template_images': template_frames,
                                   ...
                                   'search_pre_anno': search_anno['pre_bbox'],
                                   'dataset': dataset.get_name(),
                                   'test_class': meta_obj_test.get('object_class_name')})
                #return data -> 이렇게 하면 반환은 되지만 numpy array 형태임 
    ...
    def getitem_cls(self):
        ...
        while not valid:
            ...
            # 3. Make frame(template and search pair) & Data dict
            # 3-1. video set
            if is_video_dataset:
            ...
            try: 
                # 3-3-1. (for templates) get images and bounding boxes
                template_frames, template_anno, meta_obj_train = dataset.get_frames(seq_id, template_frame_ids, seq_info_dict)
                H, W, _ = template_frames[0].shape
                template_masks = template_anno['mask'] if 'mask' in template_anno else [torch.zeros((H, W))] * self.num_template_frames
                
                # 3-3-2. (for searches) get images and bounding boxes
                # positive samples
                ...
                data = TensorDict({'template_images': template_frames,
                                   ...
                                   'test_class': meta_obj_test.get('object_class_name')})
                # make data augmentation
                ##data = self.processing(data) (processing X)
                # add classification label
                data["label"] = label
                # check whether data is valid
                ##valid = data['valid'] (processing X)
    ...
    def get_frame_ids_stark(self, visible, valid):
        ...
        while None in template_frame_ids_extra or len(template_frame_ids_extra) == 0:
            ...
            for max_gap in self.max_gap:
                ...
                # we require the frame to be valid but not neccessary visible
