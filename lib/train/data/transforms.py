...
class Transform:
    ...
    def __init__(self, *transforms):
        ...
        self._valid_inputs = ['image', 'coords', 'bbox', 'mask', 'att', 'prev_coords', 'prev_bbox']
    ...
    def __call__(self, **inputs):
        ...
        '''중요 부분 : 여기서 transform 기법들의 인스턴스가 불러와진다.'''
...
class TransformBase:
    ...
    def __call__(self, **inputs):
        ...
        for var_name, var in input_vars.items():
            if var is not None:
                ...
                if var_name in ['coords', 'bbox', 'prev_coords', 'prev_bbox']:
    ...
    def transform_prev_coords(self, prev_coords, image_shape, *rand_params):
        """Must be deterministic"""
        return prev_coords
    ...
    def transform_prev_bbox(self, prev_bbox, image_shape, *rand_params) :
        if self.transform_prev_coords.__code__ == TransformBase.transform_prev_coords.__code__:
            return prev_bbox
        # [28개?] 여기임!
        prev_coord = prev_bbox.reshape(-1, 4)
        coord_all = []
        for i in range(prev_coord.shape[0]) :
            coord = prev_coord[i].view(-1, 2).t().flip(0)
            x1 = coord[1, 0]
            x2 = coord[1, 0] + coord[1, 1]
            y1 = coord[0, 0]
            y2 = coord[0, 0] + coord[0, 1]
            coords = torch.tensor([[y1, y1, y2, y2], [x1, x2, x2, x1]])
            coord_all.append(coords)
            
        coord_transf = self.transform_prev_coords(coord_all, image_shape, *rand_params)
        bbox_out = torch.Tensor([])
        for coord_t in coord_transf :
            coord_t = coord_t.flip(0)
            tl = torch.min(coord_t, dim=1)[0]
            sz = torch.max(coord_t, dim=1)[0] - tl
            bbox_out = torch.cat((bbox_out, torch.cat((tl, sz), dim=-1)), dim=-1)
        bbox_out = bbox_out.reshape(prev_bbox.shape)
        return bbox_out
...
class RandomHorizontalFlip(TransformBase):
    ...
    def transform_prev_coords(self, prev_coords, image_shape, do_flip) :
        if do_flip  :
            flip_list = []
            for coords in prev_coords :
                coords_flip = coords.clone()
                coords_flip[1, :] = (image_shape[1] - 1) - coords[1,:]
                flip_list.append(coords_flip)
            return flip_list
        return prev_coords

    def transform_coords(self, coords, image_shape, do_flip):
        '''coords : [[183, 183, 234, 234], [339, 364, 364, 339]] -> [[183, 183, 234, 234], [300, 275, 275, 300]], shape:(480, 640)'''
        if do_flip:
            coords_flip = coords.clone()
            coords_flip[1,:] = (image_shape[1] - 1) - coords[1,:]
            return coords_flip
