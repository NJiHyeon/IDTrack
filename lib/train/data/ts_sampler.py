...
class TsSampler(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 2. Sample a sequence with enough visible frames
        ...
        # 3. Sample template/search frame ids  
        if is_video_dataset:
        ...
        # 4. Sample template/search frames
        ...
        dataset_train = {'template_images': np.array(template_frames).squeeze().shape,    # 1 template images
                'template_annos': np.array(template_bbox).squeeze().shape,
                'search_images': np.array(search_frames).shape,      # (num_frames) search images
                'search_annos': np.array(search_bbox).shape,
                'seq_id': seq_id,
