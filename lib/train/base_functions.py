    ...
    for name in name_list:
        ...
        if name == "COCO17":
            if settings.use_lmdb:
                ...
                datasets.append(MSCOCOTs_lmdb(settings.env.coco_lmdb_dir, version="2017", image_loader=image_loader))
            else:
                datasets.append(MSCOCOTs(settings.env.coco_dir, version="2017", image_loader=image_loader))
        if name == "VID":
            if settings.use_lmdb:
...
def build_dataloaders(cfg, settings):
    ...
    transform_joint = tfm.Transform(tfm.ToGrayscale(probability=0.05),
                                    tfm.RandomHorizontalFlip(probability=0.5)) #뺐음 

    transform_train = tfm.Transform(tfm.ToTensorAndJitter(0.2),
                                    tfm.RandomHorizontalFlip_Norm(probability=0.5),
                                    tfm.Normalize(mean=cfg.DATA.MEAN, std=cfg.DATA.STD)) #뺐음

    transform_val = tfm.Transform(tfm.ToTensor(),
                                  tfm.Normalize(mean=cfg.DATA.MEAN, std=cfg.DATA.STD))
    
    # transform_joint = tfm.Transform(tfm.ToGrayscale(probability=0.05)) #뺐음 
    ...
    ## 2. The tracking paris processing module
    ...
    # template
        # image : [1, 3, 128, 128]
        # anno : [[0.1796, 0.2987, 0.6329, 0.3949]]
    # search
        # image : [1, 3, 256, 256]
        # anno : [[0.6313, 0.4373, 0.3411, 0.2138]]
    # dataset_train print : print('dataset', dataset_train.getitem())
