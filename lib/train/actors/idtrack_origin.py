...
class IDTrackActor(BaseActor) :
    ...
    def forward_pass(self, data) :
        ...
        # Pre box
        pre_bbox = data['search_pre_anno'] #[bs, 28] xyxy form
        ...
        pre_bbox = pre_bbox.clamp(min=(-1*magic_num), max=(1+magic_num))
        data['pre_seq_xyxy'] = pre_bbox
    ...
    def compute_losses_feat(self, pred_dict, gt_dict, return_status=True) :
        ...
        pred_feat = pred_dict["feat"] #[4,bs,802]
        
        
        #search_anno = gt_dict['search_anno'][-1] #[1,bs,4] => [bs,4] xywh form
        ...
        beta = 1
        pred = pred_feat[0:4, :, 0:self.bins*self.range] * beta
        target = seq_output[:, 0:4].to(pred_feat)
        
        out = pred.softmax(-1).to(pred)
        mul = torch.range((-1*self.magic_num+1/(self.bins*self.range)), (1+self.magic_num-1/(self.bins*self.range)), 2/(self.bins*self.range)).to(pred) #[800]
        ans = out * mul #[4, bs, 800]
        ans = ans.sum(dim=-1) #[4, bs]
        ans = ans.permute(1, 0).to(pred) #[bs, 4]
