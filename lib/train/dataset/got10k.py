...
class Got10k(BaseVideoDataset):
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        #print('seq_id', seq_id) : 2358
        #print('frame_id', frame_ids) : [671]
        #print('anno', anno) : 
        #print('anno bbox', anno['bbox'], 'seq_id', seq_id, 'frame_ids', frame_ids)
        ...
        # anno_frames의 key에 'pre_bbox' 추가
        #print('frame_ids', frame_ids[0])
        if frame_ids[0] < 7 :
            p = 7-frame_ids[0]
            anno_frames['pre_bbox'] = [torch.cat([torch.cat([anno['bbox'][0].clone()] * p), anno['bbox'][:f_id, ...].clone().reshape(1, -1).squeeze()]) for f_id in frame_ids] #[tensor([a, b, c, d, a, b, c, d, a, b, c, d...])]
        else :
            anno_frames['pre_bbox'] = [anno['bbox'][f_id-7 : f_id, ...].clone().reshape(1, -1).squeeze() for f_id in frame_ids]
